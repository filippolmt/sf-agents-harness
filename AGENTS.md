...
Their content depends on the CLI version and on the global OpenSpec profile
(`~/.config/openspec/config.json`), which decides the workflow list and whether
commands, skills, or both are written. Both are pinned in
`config/openspec-profile.json`, and `scripts/openspec-update.sh` applies them:
...
so personal settings it does not pin are preserved. It runs the pinned CLI through
`npx`, so the local `openspec` version does not affect the output. Requires `jq`
and `npx`.

Two workflows keep the trees honest:

- `.github/workflows/test-openspec-update.yml` fails a pull request whose generated
  files do not match the pinned CLI version and profile.
- `.github/workflows/openspec-update.yml` runs every Monday, bumps `cli_version` to
  the latest published OpenSpec release, regenerates, and opens a PR when anything
  changed. Scheduled runs squash-merge that PR automatically; manual dispatches leave
  it open for review.

To move to a different CLI version by hand, edit `cli_version` in
`config/openspec-profile.json` and run the script.
